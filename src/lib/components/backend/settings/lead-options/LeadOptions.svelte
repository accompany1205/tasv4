<script lang="ts">
	...
	function changeStatus(event:any, status: string) {
        const value = event.target.value;
		let _leadStatus = {};
		Object.keys(leadStatus).map((_status) => {
			if (_status !== status) {
				_leadStatus = { ..._leadStatus, [_status]: leadStatus[_status] };
			} else {
				_leadStatus = { ..._leadStatus, [value]: leadStatus[status] };
			}
		});
		leadStatus = { ..._leadStatus };
	}
	function changeReason(event:any, status: string, index: number) {
        const value = event.target.value;
		leadStatus[status][index] = value;
	}
...
{#each Object.entries(leadStatus) as [status, reasons], statusIndex}
	<div class="flex gap-4 mb-8">
		<Input
			bind:value="{status}"
			class="text-lg w-1/2 h-10 my-2 text-emerald-500"
			placeholder="Status Title"
			on:change="{(event) => changeStatus(event, status)}"
		/>

		<div class="w-1/2">
			{#each reasons as reason, index}
				<Input
					bind:value="{leadStatus[status][index]}"
					class="my-2 w-full"
					placeholder="Reason"
                    on:change="{(event) => changeReason(event, status, index)}"
				/>
			{/each}
			<Button
				on:click="{() => addReason(status)}"
				color="alternative"
				class="m-auto w-full text-lg p-1 block font-bold bg-gray-50 border-gray-300 text-emerald-500"
				>+</Button
			>
		</div>
	</div>
...
<div class="w-full flex gap-4 justify-center">
	<Button on:click="{saveLeadStatus}" class="w-52">Save</Button>

	<Button
		on:click="{addStatus}"
		color="alternative"
		class="w-52 bg-gray-50 border-gray-300 text-emerald-500"
	>
		Add Status
	</Button>
</div>
