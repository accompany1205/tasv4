<script lang="ts">
    ...
    export let btnClass = "";
    export let btnTitle = "Choose Images";
    ...
    function handleZoomChange(change: number, event:any) {
        ...
        columnsCount = Math.max(1, Math.min(7, columnsCount + change));
    }
...
<Button on:click={() => (showModal = true)} class='{btnClass}'><EditOutline/></Button>

<Modal title="{modalTitle}" bind:open={showModal} size="xl">
    <div class="w-full">
    <ButtonGroup>
        <Button on:click={(event) => handleZoomChange(1, event)} disabled={columnsCount === 7}>Zoom Out</Button>
